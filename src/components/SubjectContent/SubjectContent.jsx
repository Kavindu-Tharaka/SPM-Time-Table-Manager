...
const SubjectContent = () => {
	...
	const onSubmit = (e) => {
		...
		axios
			.post('https://time-table-manager.herokuapp.com/api/v1/subjects', {
				subjectCode,
				offeredYear,
				offeredSemester,
				subjectName,
			...
			.then((res) => {
				store.addNotification(
					buildToast('success', '', 'Subject Added Successfully')
				);
				setRefresh(true);
				clear();
			})
		...
		//     axios.patch(`https://time-table-manager.herokuapp.com/api/v1/subjects/${id}`, {
		//         subjectCode,
		//         offeredYear,
		//         offeredSemester,
		//         subjectName,
	...
	const loadData = () => {
		axios
			.get('https://time-table-manager.herokuapp.com/api/v1/subjects')
			.then((result) => {
	...
	const deleteSubject = (id) => {
		axios
			.delete(
				`https://time-table-manager.herokuapp.com/api/v1/subjects/${id}`
			)
			.then((res) => {
				console.log(res);
				setSubjectData(
				...
				swal.close();
				store.addNotification(
					buildToast('danger', '', 'Subject Deleted')
				);
			})
			...
			cell: (row) => (
				<div className='d-flex'>
					...
					<button
						...
						onClick={() => onDeleteClick(row._id, row.subjectName)}
					>
						<IoMdClose />
					</button>
				</div>
			...
			<form>
				...
				<div className='form-row'>
					<div className='form-group col'>
						...
						>
							<option name='S1' value='1'>
								S1
							</option>
							<option name='S2' value='2'>
								S2
							</option>
						</select>
					</div>
				...
				<div className='d-flex justify-content-end'>
					<button
						className='btn btn-primary wk-submit-button mt-2'
						onClick={onSubmit}
					>
						{' '}
						{'Add'}
