...
const SessionContent = () => {
	...
	const fetchData = async () => {
		...
		//grps
		try {
			const subgrpdata = await axios.get(
	...
	const onSubmit = (e) => {
		...
		axios
			.post('https://time-table-manager.herokuapp.com/api/v1/session', {
				lecturers,
				tag,
				studentGroup,
				subject,
	...
	const deleteSession = (rowID) => {
		axios
			.delete(
				`https://time-table-manager.herokuapp.com/api/v1/session/${rowID}`
			)
			.then((res) => {
				setSessionDeatils(
					sessionDetails.filter((ses) => {
						return ses._id !== rowID;
					})
				);
				swal.close();
				store.addNotification(
					buildToast('danger', '', 'Session deleted')
				);
			})
		...
		const filteredItems =
			filterText == ''
				...
				: sessionDetails.filter(
						(item) =>
							item.lecturers == `${filterText}` ||
							item.duration == `${filterText}` ||
							item.numberOfStudents == `${filterText}` ||
							item.studentGroup == `${filterText}` ||
							item.subject == `${filterText}` ||
							item.tag == `${filterText}`
				  );
			...
			cell: (row) => (
				<div className='d-flex'>
					<button
						...
						onClick={() => onDeleteClick(row._id, row.name)}
					>
						<IoMdClose />
					</button>
				</div>
							...
							<div className='form-group col'>
								...
								<div className=''>
									<select
										...
										id='student-grp'
										value={studentGroup}
										onChange={(e) => onstudentGrpChange(e)}
										className='form-control'
									>
										{tagsArr.length > 0 ? (
											tag == 'Practical' ? (
							...
							<div className='form-group col'>
								...
								<div className=''>
									<select
										...
										value={subject}
										className='form-control'
										onChange={(e) => onSubjctChange(e)}
									>
										{subjectsArr.length > 0
											? subjectsArr.map((data) => {
													return (
														<option
															key={data._id}
															value={
