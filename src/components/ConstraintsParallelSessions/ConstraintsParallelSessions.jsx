...
function ConstraintsParallelSessions() {
	...
	const [
		...
		setParallelSessionsConstraintsList,
	] = useState([]);
	...
	const addConstraint = () => {
		if (sessionBucket.length < 2) {
			...
			axios
				.post(
					'https://time-table-manager.herokuapp.com/api/v1/constraintsparallelsessions',
					{
						// year: year,
						// semester: semester,
	...
	useEffect(() => {
		...
		const loadData = () => {
			axios
				...
				.then((res) => {
					setParallelSessionsConstraintsList(
						res[2].data.data.constraintsParallelSessions
					);
	...
	return (
		<div>
			...
			<ContentHeader header={'Parallel Sessions'} />
			<div
				style={{
					marginTop: '3%',
					paddingLeft: '1%',
...
export default ConstraintsParallelSessions;
