...
function UpdateConstraintsSubGroupsDialogBox(props) {
	const [currentSubGroupName, setCurrentSubGroupName] = useState(
		props.subgroupid
	);
	const [subgroupIDbehalfOfName, setSubGroupIDBehalfOfName] = useState(
		props.subgroups.find(
			(element) => element.subgroupid === props.subgroupid
		)._id
	);
	...
	const onSubGroupChange = (e) => {
		...
		setSubGroupIDBehalfOfName(
			props.subgroups.find(
				(element) => element.subgroupid === e.target.value
			)._id
		);
	};
	...
	const editConstraint = () => {
		...
		if (to === '') {
			...
			if (!isExist) {
				axios
					.patch(
						`https://time-table-manager.herokuapp.com/api/v1/constraintssubgroups/${props.id}`,
						{
					...
					>
						{props.subgroups.map((item) =>
							...
							semester === item.subgroupid.substring(4, 5) ? (
								<option key={item._id} value={item.subgroupid}>
									{item.subgroupid}
								</option>
							) : null
						)}
